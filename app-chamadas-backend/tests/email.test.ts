...
test('confirmação usa Nexa, escapa HTML e mantém o token no link correto', () => {
  ...
  const mail = verificationEmail('<img src=x onerror=alert(1)>', token, 'https://nexa-chi-dusky.vercel.app/');
  ...
  assert.equal(new URL(link).origin, 'https://nexa-chi-dusky.vercel.app');
...
test('confirmação gera alternativas texto e HTML sem envio externo', async () => {
  ...
  const result = await transport.sendMail({
    ...
    ...verificationEmail('Ana', 'test-only', 'https://nexa-chi-dusky.vercel.app'),
  });
