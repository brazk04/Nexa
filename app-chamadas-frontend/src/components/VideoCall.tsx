...
import type { CallController, CallState } from '../lib/CallController';
import type { CallParticipant } from '../../../shared/protocol';
import { Avatar } from './Avatar';
import { Icon } from './Icon';

const PHASE_LABELS = { idle: 'Chamada encerrada', media: 'Entrando na chamada…', waiting: 'Aguardando participantes', connecting: 'Conectando chamada…', connected: 'Chamada conectada', reconnecting: 'Restabelecendo conexões…' };
function useSpeaking(stream: MediaStream | null, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    let context: AudioContext; try { context = new AudioContext(); } catch { return; }
    ...
    const samples = new Uint8Array(analyser.frequencyBinCount); let frame = 0; let last = false;
    const watch = () => { analyser.getByteFrequencyData(samples); const average = samples.reduce((total, value) => total + value, 0) / samples.length; const next = average > 16; if (next !== last) { last = next; setSpeaking(next); } frame = requestAnimationFrame(watch); };
...
export function VideoCall({ state, controller, username, selfId, selfUserId, selfAvatarUrl, ownerId, speakerId }: { state: CallState; controller: CallController; username: string; selfId?: string; selfUserId: string; selfAvatarUrl: string | null; ownerId: string; speakerId?: string }) {
  ...
  return <section className={`call-panel ${maximized ? 'is-maximized' : ''}`} aria-label="Chamada de vídeo">
    ...
    <div className="call-controls"><button aria-label={state.microphone ? 'Silenciar microfone' : 'Ativar microfone'} disabled={!ready || state.microphoneBusy} className={!state.microphone ? 'control-off' : ''} onClick={() => void controller.toggleMicrophone()}><Icon name="mic" off={!state.microphone} /><span>{state.microphoneBusy ? 'Ativando…' : state.microphone ? 'Microfone' : 'Mutado'}</span></button><button aria-label={state.camera ? 'Desligar câmera' : 'Ligar câmera'} disabled={!ready || state.cameraBusy} className={!state.camera ? 'control-off' : ''} onClick={() => void controller.toggleCamera()}><Icon name="video" off={!state.camera} /><span>{state.cameraBusy ? 'Ativando…' : state.camera ? 'Câmera' : 'Sem câmera'}</span></button><button aria-label={state.screen ? 'Parar compartilhamento' : 'Compartilhar tela'} disabled={!ready || state.sharingBusy} className={state.screen ? 'control-active' : ''} onClick={() => void controller.toggleScreen()}><Icon name="screen" /><span>{state.screen ? 'Parar tela' : 'Compartilhar'}</span></button><button aria-label={self.handRaisedAt ? 'Abaixar a mão' : 'Levantar a mão'} className={self.handRaisedAt ? 'control-active' : ''} onClick={controller.toggleHand}><Icon name="hand" /><span>Mão</span></button>
      <span className="reaction-picker">{['👍', '👏', '❤️', '😂', '🎉', '🤔'].map(emoji => <button aria-label={`Reagir com ${emoji}`} key={emoji} onClick={() => controller.sendReaction(emoji)}>{emoji}</button>)}</span><button aria-label={state.ecoMode ? 'Desativar Modo Eco' : 'Ativar Modo Eco'} className={state.ecoMode ? 'control-active' : ''} onClick={() => void controller.setEcoMode(!state.ecoMode)}><Icon name="leaf" /><span>Eco</span></button><button aria-label="Abrir participantes" onClick={() => setPanel(value => !value)}><Icon name="users" /><span>Pessoas</span></button><button aria-label={maximized ? 'Restaurar chamada' : 'Maximizar chamada'} onClick={() => setMaximized(value => !value)}><Icon name={maximized ? 'minimize' : 'maximize'} /><span>{maximized ? 'Restaurar' : 'Maximizar'}</span></button><button aria-label="Encerrar chamada" className="control-danger" onClick={() => controller.leave()}><Icon name="call" /><span>Encerrar</span></button>
    </div><p className="shortcut-hint">Atalhos: M microfone · V câmera · H mão · C chat · Shift+F maximizar · Esc restaurar</p>
  </section>;
}
function duration(startedAt: string | null) { if (!startedAt) return '00:00'; const seconds = Math.max(0, Math.floor((Date.now() - new Date(startedAt).getTime()) / 1000)); const hours = Math.floor(seconds / 3600); return [hours, Math.floor(seconds % 3600 / 60), seconds % 60].filter((_, index) => hours || index).map(value => String(value).padStart(2, '0')).join(':'); }
